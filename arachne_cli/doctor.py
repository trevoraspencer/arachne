...
os.environ.setdefault("MSWEA_GLOBAL_CONFIG_DIR", str(ARACHNE_HOME))
...
def run_doctor(args):
    ...
    # Check ~/.arachne/.env (primary location for user config)
    env_path = ARACHNE_HOME / '.env'
    if env_path.exists():
        check_ok("~/.arachne/.env file exists")
        ...
        if fallback_env.exists():
            ...
            check_fail("~/.arachne/.env file missing")
            if should_fix:
    ...
    if config_path.exists():
        check_ok("~/.arachne/config.yaml exists")
    else:
        ...
        if fallback_config.exists():
            ...
            if should_fix and example_config.exists():
                ...
                check_ok("Created ~/.arachne/config.yaml from cli-config.yaml.example")
                fixed_count += 1
            elif should_fix:
                check_warn("config.yaml not found and no example to copy from")
                manual_issues.append("Create ~/.arachne/config.yaml manually")
            else:
    ...
    if arachne_home.exists():
        ...
        if should_fix:
            ...
            check_warn("~/.arachne not found", "(will be created on first use)")
    ...
    for subdir_name in expected_subdirs:
        ...
        if subdir_path.exists():
            ...
            if should_fix:
                ...
                check_ok(f"Created ~/.arachne/{subdir_name}/")
                fixed_count += 1
            else:
    ...
    if soul_path.exists():
        ...
        if lines:
            check_ok("~/.arachne/SOUL.md exists (persona configured)")
        else:
    ...
    if memories_dir.exists():
        ...
        check_warn("~/.arachne/memories/ not found", "(will be created on first use)")
        if should_fix:
            memories_dir.mkdir(parents=True, exist_ok=True)
            check_ok("Created ~/.arachne/memories/")
            fixed_count += 1
    ...
    if state_db_path.exists():
        try:
            ...
            conn.close()
            check_ok(f"~/.arachne/state.db exists ({count} sessions)")
        except Exception as e:
            check_warn(f"~/.arachne/state.db exists but has issues: {e}")
    else:
        check_info("~/.arachne/state.db not created yet (will be created on first session)")
    ...
    hub_dir = ARACHNE_HOME / "skills" / ".hub"
    if hub_dir.exists():
    ...
    if github_token:
        ...
        check_warn("No GITHUB_TOKEN", "(60 req/hr rate limit — set in ~/.arachne/.env for better rates)")
