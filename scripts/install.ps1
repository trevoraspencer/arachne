...
function Test-Node {
    ...
    # Fallback: download binary zip to ~/.arachne/node/
...
function Copy-ConfigTemplates {
    ...
    if (-not (Test-Path $envPath)) {
        ...
        if (Test-Path $examplePath) {
            ...
            Write-Success "Created ~/.arachne/.env from template"
        } else {
            New-Item -ItemType File -Force -Path $envPath | Out-Null
            Write-Success "Created ~/.arachne/.env"
        }
    } else {
        Write-Info "~/.arachne/.env already exists, keeping it"
    }
    ...
    if (-not (Test-Path $configPath)) {
        ...
        if (Test-Path $examplePath) {
            ...
            Write-Success "Created ~/.arachne/config.yaml from template"
        }
    } else {
        Write-Info "~/.arachne/config.yaml already exists, keeping it"
    }
...
"@ | Set-Content -Path $soulPath -Encoding UTF8
        Write-Success "Created ~/.arachne/SOUL.md (edit to customize personality)"
    }
    
    Write-Success "Configuration directory ready: ~/.arachne/"
    
    # Seed bundled skills into ~/.arachne/skills/ (manifest-based, one-time per skill)
    Write-Info "Syncing bundled skills to ~/.arachne/skills/ ..."
    $pythonExe = "$InstallDir\venv\Scripts\python.exe"
    if (Test-Path $pythonExe) {
        try {
            & $pythonExe "$InstallDir\tools\skills_sync.py" 2>$null
            Write-Success "Skills synced to ~/.arachne/skills/"
        } catch {
