...
def test_arachne_cli_init_does_not_eagerly_resolve_runtime_provider(monkeypatch):
    cli = _import_cli()
    calls = {"count": 0}
...
def test_cmd_model_falls_back_to_auto_on_invalid_provider(monkeypatch, capsys):
    monkeypatch.setattr(
        "arachne_cli.config.load_config",
        lambda: {"model": {"default": "gpt-5", "provider": "invalid-provider"}},
    )
    monkeypatch.setattr("arachne_cli.config.save_config", lambda cfg: None)
    monkeypatch.setattr("arachne_cli.config.get_env_value", lambda key: "")
    monkeypatch.setattr("arachne_cli.config.save_env_value", lambda key, value: None)
